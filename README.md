...
Ve a https://console.cloud.google.com/ y crea una cuenta
Activa la facturación en https://console.cloud.google.com/billing
Crea un nuevo proyecto y anota el PROJECT_ID
...
4. Crear la VM
bashgcloud compute instances create practica-creativa \
  --zone us-central1-a \
...
bashgcloud compute ssh practica-creativa --zone us-central1-a --project <tu-project-id>
...
newgrp docker
Java 17 y SBT
bashsudo apt-get install -y openjdk-17-jdk
...
Python 3
bashsudo apt-get install -y python3 python3-pip python3-venv
Spark 4.1.1
bashcurl -LO https://archive.apache.org/dist/spark/spark-4.1.1/spark-4.1.1-bin-hadoop3.tgz
...
bashgit clone https://github.com/aNogalesHurtado/practica-creativa-ibdn.git
...
4. Descargar datos
bashbash resources/download_data.sh

5. Compilar el JAR de Spark
bashcd flight_prediction
sbt package
cd ..
...
bashcat > /tmp/create_iceberg.py << 'EOF'
...
bashdocker exec -it cassandra cqlsh -e "
...
bashdocker-compose restart spark-predictor flask
...
bashexport PROJECT_ID=<tu-project-id>
...
gcloud services enable containerregistry.googleapis.com
3. Añadir repositorio de Google Cloud SDK e instalar kubectl y el plugin de GKE
bashecho "deb [signed-by=/usr/share/keyrings/cloud.google.gpg] https://packages.cloud.google.com/apt cloud-sdk main" | sudo tee /etc/apt/sources.list.d/google-cloud-sdk.list
...
bashgcloud container clusters create practica-creativa-k8s \
  --zone us-central1-a \
  --num-nodes 2 \
  --machine-type e2-standard-4 \
  --project $PROJECT_ID
...
bashgcloud container clusters get-credentials practica-creativa-k8s \
...
bashsed -i 's|\$PROJECT_ID|'$PROJECT_ID'|g' k8s/*.yaml
...
bashkubectl exec -it $(kubectl get pod -l app=kafka \
...
bashkubectl exec -it $(kubectl get pod -l app=cassandra \
...
bashkubectl port-forward svc/cassandra 9042:9042 &
sleep 5
source env/bin/activate
...
bashkubectl port-forward svc/minio 9000:9000 &
sleep 3
...
bashkubectl rollout restart deployment/spark-predictor
...
Acceder a Grafana
bashkubectl --namespace monitoring get services | grep grafana
